...
const TopLabTechniques = () => {
    const { labTechniques } = useContext(AppContext);
    const navigate = useNavigate();
    
    return (
        <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-2 text-gray-800">Top Lab Tests to Book</h1>
            <p className="text-gray-600 text-center mb-10 max-w-2xl mx-auto">Simply browse through our extensive list of trusted lab tests.</p>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {labTechniques && labTechniques.slice(0, 8).map((item, index) => (
                    <div onClick={() => {
                        scrollTo(0,0);
                        navigate(`/lab-booking/${item._id}`);
                        console.log("Navigating to lab technique with ID:", item._id);
                    }} key={index} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 cursor-pointer">
                        <img 
                            src={item.image} 
                            alt={item.name} 
...
export default TopLabTechniques;